...
const AdminInModal = ({ onClose }) => {
  ...
  const onSaveHistory = (payload = {}) => {
    const request = { ...payload, creator: admin, historyType: "NEW" };
  ...
  const isSubmitDisabled = !carNumber || !selectedCompany || !selectedType || !selectedWarehouse || !quantity;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
      <div className="bg-white p-6 rounded shadow w-[400px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-2">
          <input type="text" placeholder="차량번호" value={carNumber} onChange={(e) => handleCarNumberChange(e.target.value)} className="border p-2 w-full rounded" />
          ...
          <select value={selectedCompany} onChange={(e) => setSelectedCompany(e.target.value)} className="border p-2 w-full rounded">
          ...
          <select value={selectedType} onChange={(e) => setSelectedType(e.target.value)} className="border p-2 w-full rounded">
          ...
          <select value={selectedWarehouse} onChange={(e) => setSelectedWarehouse(e.target.value)} className="border p-2 w-full rounded">
          ...
          <div>
            ...
            <input type="number" min="1" placeholder="수량" value={quantity} onChange={(e) => handleQuantityChange(e.target.value)} className="border p-2 w-full rounded" />
            {quantityWarning && <p className="text-xs text-yellow-500 mt-1">{quantityWarning}</p>}
          </div>
          ...
          <div className="border p-3 rounded mt-2">
            ...
            <input type="date" value={dateIn} onChange={(e) => setDateIn(e.target.value)} className="border p-2 w-full rounded mb-2" />

            <div className="flex justify-between items-center mb-1">
              <span className="text-sm font-medium text-gray-700">출고일</span>
              <span className="text-xs text-red-500">※ 미입력 시 없음으로 자동 처리됩니다.</span>
            </div>
            <input type="date" value={dateOut} onChange={(e) => setDateOut(e.target.value)} className="border p-2 w-full rounded mb-2" />

            <textarea placeholder="메모 (선택)" value={memo} onChange={(e) => setMemo(e.target.value)} className="border p-2 w-full rounded" />
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded p-6 w-[360px] shadow">
            ...
            <ul className="text-sm space-y-1 mb-4">
              ...
              <li>창고: {convertWarehouseName(confirmedPayload.warehouse)}</li>
            ...
            <div className="flex justify-between">
              ...
              <button className="text-gray-500 hover:underline" onClick={() => setShowConfirmModal(false)}>
                아니오
              </button>
            </div>
          </div>
      ...
      {showLocationModal && <StockLocationModal stockInfo={confirmedPayload} onSubmit={handleFinalSubmit} onCancel={() => setShowLocationModal(false)} />}
    </div>
  );
};
